...
extension, with no version filter. `CCM_Tool_by_Son_v0.54.1.zip` therefore
contained:

- `MCE_CCM_v0.53.3.pyt` and `MCE_CCM_v0.54.0.pyt` — **two obsolete
  toolboxes**, which appear in ArcGIS Pro's Catalog beside the real one and
  display old version labels while executing current code;
- `MCE_CCM_Tool_v0.53.3_User_Manual.docx` and `…v0.54.0…docx` — superseded
  manuals still carrying the pre-rebrand product name;
- twelve orphan `.pyt.xml` sidecars;
- `~$E_CCM_Tool_v0.54.0_User_Manual.docx` — a Word lock file.

A user unzipping the release saw three toolboxes and three manuals. Fixed by:

- `should_include()` — screens Office lock files, `MCE_CCM*` pre-rebrand
...
`CCM_Tool_by_Son_v0.54.1.pyt.xml` internally still read
`<toolbox name="CCM_Tool_by_Son_v0.50.1">` / `<resTitle>…v0.50.1</resTitle>`.
