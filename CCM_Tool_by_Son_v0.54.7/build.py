...
---------------
  - FIX: the packager walked the whole project folder and included every
    ...
    (MCE_CCM_v0.53.3.pyt, MCE_CCM_v0.54.0.pyt), two superseded user
    manuals, twelve orphan .pyt.xml sidecars, and a Word lock file
    (~$...docx).  A user unzipping it saw three toolboxes and three
    manuals.  Files are now screened by should_include(), which rejects
    Office lock files, pre-rebrand MCE_CCM* artifacts, and anything
    carrying a _v<version> tag that is not the current release.
  - FIX: PY_FILES hardcoded "CCM_Tool_by_Son_v0.54.1.pyt" despite the
...
TOOLBOX_NAME = f"CCM_Tool_by_Son_v{VERSION}.pyt"
...
EXCLUDE_PREFIXES = ("~$", ".~")
# Pre-rebrand artifacts.  "MCE CCM Tool" became "CCM Tool by Son" in v0.54.1;
# anything still carrying the old product name is by definition stale.
EXCLUDE_SUBSTRINGS = ("MCE_CCM",)
...
# always ship.  CHANGELOG_v0.45.md .. CHANGELOG_v0.54.md are the project's
...
def should_include(fname):
    ...
    for sub in EXCLUDE_SUBSTRINGS:
        if sub in fname:
            return False, f"pre-rebrand artifact ('{sub}')"
