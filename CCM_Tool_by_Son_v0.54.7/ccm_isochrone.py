...
VERSION = "0.54.7"  # v0.54.7 — no logic change here; fixed a smoke-test detection bug (see CHANGELOG_v0.54.md).
# v0.54.6 — Reliability follow-up: a real ArcGIS Pro 3.7.1 re-run confirmed
#           the v0.54.5 mitigation (CalculateStatistics + single-threaded
#           retry against the SAVED raster path) fires correctly but does
#           NOT resolve ERROR 160333 — the retry hits the identical error.
#           Two further changes: (1) _reclassify_with_retry()'s primary
#           attempt now runs on the in-memory cost_dist Raster object
#           instead of re-reading it from disk immediately after saving
#           (a plausible geodatabase catalog-timing trigger), with the
#           v0.54.5 stats+single-thread retry kept as a second-line
#           fallback; (2) generate_isochrones() now falls back to the
#           already-proven vector method if the Spatial Analyst path fails
#           for any reason, so a real ArcGIS/licence issue no longer means
#           no isochrone output at all.
# v0.54.5 — Reliability fix: ERROR 160333 ("The table was not found") could
#           surface on Reclassify immediately after DistanceAccumulation, a
#           known but poorly-documented ArcGIS Pro raster issue (Esri KB
#           000027676; Esri Community thread on Pro 3.4.2 confirms no single
#           deterministic root cause is published). Two mitigations: (1)
#           cost_dist.save() now runs INSIDE the same EnvManager block as
#           DistanceAccumulation, since arcpy.sa rasters are evaluated lazily
#           and the previous scoping let the actual write happen under a
#           different parallel-processing setting than intended; (2) new
#           _reclassify_with_retry() rebuilds raster statistics and retries
#           once, single-threaded, if ERROR 160333 is hit.
# v0.46+ — Performance: DistanceAccumulation now runs inside
#           arcpy.EnvManager(parallelProcessingFactor="100%") to use all CPU
#           cores on large rasters (EnvManager auto-resets on exit).
# v0.46 — Bug fixes:
...
def _reclassify_with_retry(cost_dist, cost_dist_path: str, remap, reclass_path: str):
    """
    Run Reclassify against the in-memory cost-distance raster, falling back
    to a saved-path retry against ERROR 160333 ("The table was not found").

    ERROR 160333 is a real but poorly-understood ArcGIS Pro raster error —
    Esri's own KB article (000027676) only documents an unrelated cause
    (pre-upgrade file geodatabases), and an Esri Community thread on Pro
    3.4.2 ("appears frequently without a clear reason") confirms there is no
    single, deterministic root cause published.

    History:
      v0.54.5 mitigated this with CalculateStatistics + a single-threaded
      retry against the SAVED raster path. Confirmed by a real ArcGIS Pro
      3.7.1 run: the retry fires correctly (the warning below appeared) but
      hit the IDENTICAL error again — so stale statistics / parallel tiling
      on the saved dataset is not the (whole) story.

      v0.54.6 changes the primary attempt: Reclassify now runs directly on
      the in-memory `cost_dist` Raster object returned by
      DistanceAccumulation/CostDistance, instead of re-opening the just-
      saved path from the scratch geodatabase. Immediately re-reading a
      raster by path right after writing it is a plausible trigger for
      "table was not found" — a geodatabase's catalog/business-table entry
      for a brand-new dataset is not guaranteed to be immediately visible to
      the very next tool that queries it. Operating on the in-memory object
      skips that round-trip entirely. The v0.54.5 mitigation (rebuild
      statistics, retry once single-threaded against the saved path) is
      kept as a second-line fallback in case the in-memory attempt also
      hits ERROR 160333 — a different code path, and free unless the
      primary attempt has already failed.

    If both attempts fail, the exception from the second attempt is raised
    unchanged. The caller (generate_isochrones()) then falls back to the
    vector method rather than producing no isochrone output at all;
    ccm_step3_advanced.py's existing handling (degrade to a warning, not a
    crash) remains the last line of defence if even that fails.
    """
    import arcpy.sa as sa
    try:
        reclass = sa.Reclassify(cost_dist, "Value", remap, "NODATA")
        reclass.save(reclass_path)
        return reclass
    except arcpy.ExecuteError as e:
        if "160333" not in str(e):
            raise
        arcpy.AddWarning(
            "[CCM Isochrone] Reclassify hit ERROR 160333 on the in-memory "
            "cost-distance raster ('table was not found' — a known, non-"
            "deterministic ArcGIS Pro raster issue). Falling back to the "
            "saved raster path, rebuilding statistics, and retrying once, "
            "single-threaded …"
        )
        try:
            arcpy.management.CalculateStatistics(cost_dist_path)
        except Exception as _stats_e:
            arcpy.AddWarning(
                f"[CCM Isochrone] CalculateStatistics retry step failed: {_stats_e}"
            )
        with arcpy.EnvManager(parallelProcessingFactor="0"):
            reclass = sa.Reclassify(cost_dist_path, "Value", remap, "NODATA")
            reclass.save(reclass_path)
        arcpy.AddMessage("[CCM Isochrone] Reclassify fallback retry succeeded.")
        return reclass
...
def _generate_isochrones_sa(
    ...
    if _use_dist_accum:
        ...
        # parallelProcessingFactor="100%" directs ArcGIS to use all available
        # CPU cores for raster traversal — up to 50× faster on large rasters.
        # EnvManager is a context manager; the environment resets automatically
        # on exit, so this does not bleed into any subsequent geoprocessing.
        # v0.54.5 fix: arcpy.sa rasters are evaluated LAZILY, so the actual
        # computation can be deferred until .save() forces materialisation.
        # .save() now stays INSIDE this block so the parallel-processing env
        # is still active when the raster is actually written — saving
        # outside it let the write happen under a different, inconsistent
        # setting, one known trigger for ERROR 160333 on the Reclassify below.
    ...
    # v0.54.6: primary attempt now runs on the in-memory cost_dist Raster
    # object rather than re-reading cost_dist_path from disk — see
    # _reclassify_with_retry()'s docstring for why. Only reclass_path (the
    # saved dataset) is used below, so the returned Raster object is
    # discarded.
    _reclassify_with_retry(cost_dist, cost_dist_path, remap, reclass_path)
...
def generate_isochrones(
    ...
    if _check_spatial_analyst():
        try:
            ...
            # v0.54.6: the Spatial Analyst path (DistanceAccumulation +
            # Reclassify) can hit ERROR 160333 ("The table was not found")
            # — a real, Esri-acknowledged but non-deterministic ArcGIS Pro
            # raster issue with no published root cause and no known fix;
            # two independent mitigations inside _reclassify_with_retry()
            # already ran and both failed if this is reached. Rather than
            # return no isochrone output at all, fall back to the vector
            # method — licence-independent, already used automatically
            # when Spatial Analyst isn't available, and proven by its own
            # code path. It is less precise (time bands are bounded by
            # polygon edges, not raster cells) but real output beats none.
            arcpy.AddWarning(
                f"[CCM Isochrone] Spatial Analyst method failed ({exc}); "
                "falling back to the vector method so an isochrone is "
                "still produced. This result uses polygon boundaries "
                "rather than raster cells and may be less precise."
            )
...
class CCMIsochroneTool:
    ...
    def execute(self, parameters, messages):
        ...
        # Indices: 0=speed_fc        # Indices: 0=speed_fc, 1=speed_field, 2=start_point, 3=start_display,
        #          4=time_bands, 5=cell_size, 6=output_fc
        speed_fc  = parameters[0].valueAsText
        speed_fld = parameters[1].valueAsText
        start_raw = parameters[2].valueAsText
        bands_raw = parameters[4].valueAsText or "30;60;120;240"
        cell_size = float(parameters[5].value or 50)
        output_fc = parameters[6].valueAsText

        if _coords_mod:
            lat, lon = _coords_mod.any_to_latlon(start_raw)
        else:
            parts = [p.strip() for p in start_raw.replace(",", " ").split() if p.strip()]
            lat, lon = float(parts[0]), float(parts[1])

        time_bands = [int(x.strip()) for x in bands_raw.split(";") if x.strip()]

        result_fc = generate_isochrones(
            speed_surface_fc = speed_fc,
            speed_field      = speed_fld,
            start_point      = (lat, lon),
            time_bands_min   = time_bands,
            cell_size_m      = cell_size,
            output_fc        = output_fc,
        )

        arcpy.AddMessage("=" * 62)
        arcpy.AddMessage("  ISOCHRONE COMPLETE")
        arcpy.AddMessage(f"  Output : {result_fc}")
        arcpy.AddMessage(f"  Bands  : {time_bands} min")
        arcpy.AddMessage("  >>> Open the layer in ArcGIS Pro to view travel time zones.")
        arcpy.AddMessage("=" * 62)
