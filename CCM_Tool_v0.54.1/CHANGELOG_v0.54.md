...
- Toolbox renamed `MCE_CCM_v0.54.0.pyt` → `CCM_Tool_v0.54.1.pyt`
  (all `.pyt.xml` sidecars renamed to match); `toolname` constant
  `"MCE_CCM_Tool"` → `"CCM_Tool"`; toolbox `alias` `"MCECCMTool"` →
  `"CCMTool"`.
- `ccm_map_display.MAP_NAME` constant `"MCE_CCM_MAP"` → `"CCM_TOOL_MAP"`
...
- Release zip renamed `MCE_CCM_Tool_v<VERSION>.zip` → `CCM_Tool_v<VERSION>.zip`
  (`build.py`); user manual renamed to `CCM_Tool_v0.54.1_User_Manual.docx`.
...
### User Manual (`CCM_Tool_v0.54.0_User_Manual.docx`)
...
- Section 3.3's Data Requirements table and 4.1's Core Parameters table
  ...
  the new warnings, plus a stale `CCM_Tool_v0.51.0.pyt` filename reference
