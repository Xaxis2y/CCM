...
# CCM Tool by Son — working rules for AI sessions
...
2. Toolbox filename `CCM_Tool_by_Son_v<ver>.pyt` + ALL `.pyt.xml` sidecars (rename)
...
7. **User manual** — `CCM_Tool_by_Son_v<ver>_User_Manual.docx`: title-page
   version, section 10.4 version-history row, and content updates when the
   change is user-facing
8. Rebuild `CCM_Tool_by_Son_v<ver>.zip` via `python build.py` (version is read
