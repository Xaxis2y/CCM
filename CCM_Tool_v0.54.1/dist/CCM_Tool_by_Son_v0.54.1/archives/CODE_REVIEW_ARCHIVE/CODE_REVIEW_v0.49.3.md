...
# Logic Review — CCM Tool by Son v0.49.3
...
| Step 3 | dedicated `CCM_TOOL_BY_SON_MAP` | ~250 lines of inline hard-coded renderer logic |
...
3. **Pick one target-map policy.** Step 0 uses the active map, Step 3 creates `CCM_TOOL_BY_SON_MAP`. Suggest: all CCM outputs go to `CCM_TOOL_BY_SON_MAP`; Step 0 source data goes to the active map (it is input, not result) — but document this in the tool descriptions.
