...
- Toolbox renamed `CCM_Tool_v0.49.3.pyt` → `CCM_Tool_v0.50.0.pyt`
...
- Full-repo version sweep: every module `VERSION`, the toolbox filename
  (`CCM_Tool_v0.50.1.pyt`), all `.pyt.xml` sidecars (the stale v0.49.2
...
- All module `VERSION` constants → `0.50.2`; toolbox renamed
  `CCM_Tool_v0.50.1.pyt` → `CCM_Tool_v0.50.2.pyt` (sidecar XMLs, `build.py`,
  README, PROJECT_STATUS, test assertions updated).
- User manual updated: `CCM_Tool_v0.50.2_User_Manual.docx` (title page,
