...
- **Copyright headers simplified.** Every module header, the README copyright
  ...
  unit, and organizational attribution and the proprietary /
  authorized-use boilerplate were removed.
- Version bumped 0.53.2 → 0.53.3 across all modules, the `.pyt`
  (`CCM_Tool_v0.53.3.pyt`) and its `.pyt.xml` sidecars, `build.py`, the test
...
- Version bumped 0.53.1 → 0.53.2 across all modules, the `.pyt`
  (`CCM_Tool_v0.53.2.pyt`) and its `.pyt.xml` sidecars, `build.py`, the test
  suite, README / PROJECT_STATUS / TASKS, and the user manual.
...
- **Removed the standalone `MGCP Data Loader/` folder** (`LoadMGCPData`
  ...
  drift risk.
- **Removed orphaned sidecar** `CCM_Tool_v0.49.2.CCMStep0MGCPTool.pyt.xml`
  (leftover from an old version; was being zipped into releases).
- **Removed superseded user manuals** `CCM_Tool_v0.46 / v0.49 / v0.50.2
...
- All module `VERSION` constants → `0.53.1`; toolbox renamed
  `CCM_Tool_v0.53.0.pyt` → `CCM_Tool_v0.53.1.pyt` (sidecars, `build.py`,
