...
Anaconda / ArcGIS Pro conda prompt (preferred — copy-paste this whole block):
    ...
    cd /d "C:\\Users\\son.es\\Documents\\ES_Project\\NEW_CCM_Tool\\CCM_Tool_v0.54.1"
...
SKIP.  To force them to run, use the ArcGIS Pro Python window instead:
    ...
    exec(open(r"C:\\Users\\son.es\\Documents\\ES_Project\\NEW_CCM_Tool\\CCM_Tool_v0.54.1\\tests\\verify_v0544.py").read())
...
def test_toolbox(arcpy):
    ...
    pyt = os.path.join(_ROOT, "CCM_Tool_v%s.pyt" % VERSION)
...
def test_folder_hygiene():
    ...
    junk = [f for f in os.listdir(_ROOT) if f.startswith("~$")]
    log("  stale/lock     : %s" % (junk or "none"))
    if junk:
        problems.append("%d stale file(s)" % len(junk))
    sidecar = os.path.join(_ROOT, "CCM_Tool_v%s.pyt.xml" % VERSION)
...
def main():
    log("#" * 74)
    log("#  CCM Tool v%s — ArcGIS Pro VERIFICATION" % VERSION)
