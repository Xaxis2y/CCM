...
# CHANGELOG — v0.55.1 (2026-08-06)

## Version-bump release — local-testing convenience files

No toolbox/geoprocessing logic changed. Two files were added to the
project:

- **`QUICK_START.html`** — a styled, single-file HTML rendering of
  `QUICK_START.md`, for easier on-screen reading than plain Markdown.
  Same content, same section order. Now packaged in the release zip —
  `build.py`'s `INCLUDE_PATTERNS` gained `.html`.
- **`CCM_anaconda_environment.bat`** — creates/verifies the dedicated
  `ccm_v055_test` conda environment (clone of `arcgispro-py3`, pytest
  installed into the clone only, arcpy import + install-info check) as a
  standalone step, separate from `RUN_LOCAL_VERIFICATION.bat`, which runs
  the actual test suite. `RUN_LOCAL_VERIFICATION.bat` already detects and
  reuses this same environment, so running this script first only saves
  the clone wait on a later, longer test run — it changes nothing about
  what gets tested. Not packaged in the release zip (`.bat` was and
  remains absent from `INCLUDE_PATTERNS`), matching the existing,
  unchanged treatment of `RUN_LOCAL_VERIFICATION.bat`.

### Also in this release

Before this version bump, v0.55.0 itself was given a thorough scrub pass
(no version change at the time, since none had been requested yet):

- Removed every remaining reference to the pre-rename organization name
  and product-name suffix from all documents, code, archived changelogs,
  and the User Manual, including historical/changelog narrative — not
  just current-state text.
- Fixed a privacy leak: `Vehicle_Data/Vehicles_Can.csv.xml` carried
  embedded ArcGIS geoprocessing lineage metadata naming a real Windows
  username and an old product-folder path. Stripped down to the bare
  Esri metadata block.
- Verified the exact copyright header —
  `SPDX-License-Identifier: GPL-2.0-or-later` /
  `Copyright (c) 2026 Eui Soo SON` — is present, byte-consistent, and
  free of any trailing suffix across all 27 Python files, the `.pyt`,
  the `.pyt.xml` `idCredit` field (which was missing the `Copyright (c)
  2026` prefix — fixed), the manual's title page, and every top-level
  doc. Added the missing standard header comment to `CHANGELOG_v0.54.md`
  and this file, for consistency with every other changelog in
  `archives/CHANGELOG_HISTORY/`.
- Re-verified clean: `build.py` 21/21 files OK, `pytest` 165 passed / 3
  skipped (4 skipped including the arcpy-smoke suite), pyflakes zero
  undefined names, and a whole-tree + in-zip + in-docx grep/scan sweep
  returning zero residual hits.

### Version bump

- Version bumped 0.55.0 → 0.55.1 across every module `VERSION` constant,
  the `.pyt` `toolversion`, the `.pyt` filename and all `.pyt.xml`
  sidecars (renamed), `build.py`, the test suite (including the version
  assertions and the `CCM_Tool_v<ver>.pyt` path used by
  `tests/test_ccm.py`), `README.md`, `PROJECT_STATUS.md`, `TASKS.md`,
  `QUICK_START.md` / `QUICK_START.html`, `RUN_LOCAL_VERIFICATION.bat`,
  `CCM_anaconda_environment.bat`, and the user manual (title page +
  Section 10.4 Version History).
- `tests/verify_v0544.py` intentionally left at its own pinned `0.54.4`
  — it verifies that specific historical fix by name and is not part of
  the current-version bump chain (same as every prior release).

### Verification

- `ast.parse` + pyflakes undefined-name scan clean on all 19 modules,
  `build.py`, and the `.pyt`.
- `python build.py` — 21/21 files OK, stale-toolbox guard passes,
  `CCM_Tool_v0.55.1.zip` built.
- `pytest tests/test_ccm.py tests/test_v050.py` — 165 passed / 3 skipped.
  `pytest tests/` — 165 passed / 4 skipped.
- Whole-tree, in-zip, and in-docx sweep for the pre-rename organization
  name / product-name suffix / stale `0.55.0` references — zero hits.
- The real-ArcGIS-Pro smoke tests were not re-run against a licensed
  install as part of this bump (no code they exercise changed); re-run
  them via `RUN_LOCAL_VERIFICATION.bat` before production use, per
