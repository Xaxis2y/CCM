...
`CCM_Tool_v0.55.1_User_Manual.docx`.
...
- The `CCM_Tool_v0.55.1` folder on disk (this folder)
...
3. Navigate to this folder and select `CCM_Tool_v0.55.1.pyt`.
...
- `ERROR 000384` (Union licence limit) — fixed as of v0.54.4; make sure
  you're on v0.55.1, not an older unpatched copy.
...
To set up (or just verify) that dedicated environment on its own first —
useful before a longer test run, or just to confirm arcpy imports
correctly — run `CCM_anaconda_environment.bat`; `RUN_LOCAL_VERIFICATION.bat`
will detect and reuse the environment it creates.
