...
(e.g. UTM) — see `CCM_Tool_v0.55.1_User_Manual.docx` Section 3.4 for why,
...
| File | Role |
|---|---|
| `CCM_Tool_v0.55.1.pyt` | ArcGIS Python Toolbox entry point — registers Steps 0-4 (Step 4 = the standalone Vehicle Compare tool); shows a stub error tool if a module fails to import. |
...
See `CCM_Tool_v0.55.1_User_Manual.docx` for the full manual —
...
for release notes. In short: **v0.55.1 adds a styled HTML Quick Start and
a standalone Anaconda-environment setup script** on top of the v0.55.0
consolidation release — no toolbox/geoprocessing behaviour changed.
