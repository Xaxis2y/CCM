...
# -*- coding: utf-8 -*-
"""
CCM_Tool_v0.56.0.pyt — ArcGIS Python Toolbox entry point
====================================================
Cross-Country Mobility (CCM) Assessment Tool  v0.56.0
...
v0.56.0 changes (this file):
  - Filename/version bump only; no logic in this entry-point file changed.
    The release is a Step 0 (Load MGCP Data) usability release, in
    ccm_step0_mgcp.py and ccm_mgcp_catalog.py:
      * Map layers now group into Point / Line / Polygon by default (new
        "Group Layers in Map By" parameter), in cartographic draw order,
        instead of one group per MGCP theme.
      * "Unknown feature (XXnnn)" is gone.  Uncatalogued FACC codes fall
        back to their 2-letter FACC category (correct theme, generic name)
        and code-less TRD 4.x names (HydrographySrf, ...) are classified by
        name keyword.  Step 0 lists what fell back and writes an editable
        mgcp_catalog_user.csv template beside the output GDB.
      * Imported feature classes keep their MGCP code name but gain a
        readable alias ("Building (AL015)").
      * Optional .lyrx group templates and scale-dependent drawing for
        dense detail layers.
      * Unknown-CRS repair now runs BEFORE reprojection, with a post-run
        sweep and one consolidated warning.
    See CHANGELOG_v0.56.md.

v0.55.1 changes (this file):
  - Filename/version bump only. Added two local-testing convenience files
    alongside the toolbox: QUICK_START.html (a styled HTML version of
    QUICK_START.md) and CCM_anaconda_environment.bat (creates/verifies the
    dedicated Anaconda environment used by RUN_LOCAL_VERIFICATION.bat, as
    a separate one-time setup step). No toolbox/geoprocessing logic
    changed. See CHANGELOG_v0.55.md.
...
v0.55.0 changes (this file):
  - Filename/version bump only. v0.55.0 reconciles two divergent copies of
    ...
    beyond what v0.54.2-v0.54.7 already shipped. See CHANGELOG_v0.55.md.
