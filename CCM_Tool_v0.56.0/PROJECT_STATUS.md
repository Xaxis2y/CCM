...
*Current: **v0.56.0** — August 7, 2026 (see CHANGELOG files).*

> v0.56.0: Step 0 (Load MGCP Data) usability release — no Step 1/2/3/4
> geoprocessing logic changed. (a) Map layers now group into three groups,
> **Point / Line / Polygon**, in cartographic draw order, via a new
> "Group Layers in Map By" parameter (index 16, default Geometry); the old
> group-by-theme / group-by-GDB checkboxes (9, 13) are kept but inert
> unless the new parameter is set to its legacy sentinel. (b) The
> `Unknown feature (XXnnn)` label is gone: `ccm_mgcp_catalog.lookup()` now
> resolves through user override -> exact FACC code -> **2-letter FACC
> category** -> **name keyword** -> none, reporting which tier fired in a
> new `match` key. Category matches give the correct theme with a generic
> name and deliberately infer no `ccm_role`. No feature names were invented
> for codes that could not be verified against an authoritative TRD
> catalogue. (c) Step 0 lists the codes that only reached a category match
> and writes an editable `mgcp_catalog_user.csv` beside the output GDB
> (parameter 17). (d) Imported FCs keep their MGCP code name but gain a
> readable alias, `Building (AL015)` (parameter 18). (e) Optional `.lyrx`
> group templates (19) and a scale threshold for dense line/polygon layers,
> roads exempt (20). (f) Unknown-CRS repair now runs BEFORE reprojection
> (it used to run after `CopyFeatures`, i.e. too late when an Output CRS
> was set), plus a post-run sweep and one consolidated warning.
> (g) `soil_preprocess_concept.html` gained a light theme with a toggle.
> (h) `build.py` no longer double-packages a release zip unpacked inside
> the project folder. See CHANGELOG_v0.56.md.

> v0.55.1: version-bump-only release. Added `QUICK_START.html` (a styled
> HTML rendering of `QUICK_START.md`, now packaged in the release zip —
> `build.py`'s `INCLUDE_PATTERNS` gained `.html`) and
> `CCM_anaconda_environment.bat` (a local-only script that creates/verifies
> the dedicated `ccm_v055_test` conda environment on its own, as a
> companion to `RUN_LOCAL_VERIFICATION.bat`; not packaged, same as that
> file). No toolbox/geoprocessing logic changed. See CHANGELOG_v0.55.md.
