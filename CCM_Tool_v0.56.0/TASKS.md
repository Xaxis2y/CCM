...
*Last updated: 2026-08-07 (session 11) | Current version: v0.56.0*
...
| 29 | v0.56.0 Step 0 usability (geometry groups + no "Unknown feature") | Reported symptom: loading a full MGCP delivery worked, but the Contents pane was unusable — one group per MGCP theme (a dozen groups), and a large share of layers labelled `Unknown feature (XXnnn)`. **Grouping:** new appended parameter 16 "Group Layers in Map By" (Geometry / Theme / GDB Name / None / legacy), default **Geometry**, builds exactly three groups — Point, Line, Polygon — inserted in reverse so the Contents pane reads Point→Line→Polygon top-down (points must draw above polygons). Geometry comes from `Describe().shapeType`, falling back to MGCP/DIGEST name suffixes (`...Pnt`/`...Crv`/`...Srf`, `_P`/`_L`/`_A`, `AP030L`). `_try_create_group_layer()` now reuses a same-named group instead of stacking duplicates on re-run, and inserts at TOP rather than AUTO_ARRANGE. Legacy params 9/13 retained (inert unless the legacy sentinel is chosen) so no saved model breaks; all new params appended, indices 0-15 untouched. **"Unknown":** `lookup()` now resolves through 5 tiers — user override → exact FACC code → **2-letter FACC category** → **name keyword** → none — reporting the tier in a new `match` key. The category tier is the real fix: the FACC category structure is stable across TRD revisions, so `AP999` is reliably a "Road / track feature (Transportation)". No `ccm_role` is inferred there — an unverified layer must not feed the mobility model. The keyword tier catches MGCP TRD 4.x thematic names (`HydrographySrf`, `TransportationGroundCrv`) that carry no FACC code and previously all collapsed into theme `Other`; roles are inferred there only for soil and for **contour specifically** (an `elevation`-wide match would have given a spot-height layer `ROLE_CONTOURS` and silently produced a wrong Step 1 slope surface). Web searches for an authoritative full MGCP TRD 4.x feature catalogue returned nothing usable (NGA GAIT sources on GitHub fetched empty), so **no feature names were invented** — only ~17 well-attested codes were added (`AA050`, `AF030`, `AF040`, `AJ050`, `AJ051`, `AJ110`, `AK030`, `AK120`, `AL208`, `AP040`, `AP041`, `AQ070`, `AQ125`, `AQ150`, `AQ170`, `AT005`, `ZD020`) and the rest is covered by the category tier plus a new user-editable `mgcp_catalog_user.csv` override (param 17) whose template Step 0 pre-seeds with exactly the codes that fell back, preserving rows already filled in. **Also:** readable GDB aliases, code name untouched (param 18); `.lyrx` group templates (19); scale threshold for dense line/polygon layers with road/rail exempt (20); layers always renamed to their catalog label, not only in theme mode; manifest gains `match`, `geometry_group`, `unclassified_codes`; classification-by-geometry summary in the log. **CRS ordering bug fixed:** with an Output CRS set, arcpy reprojects on copy, but a source with no `.prj` is undefined — and the WGS84 repair ran AFTER `CopyFeatures`, too late. WGS84 is now assigned to the SOURCE first, with a loud warning if the source is read-only, plus a post-run sweep for outputs left undefined by an APPEND, and the per-FC `[NO CRS]` warnings consolidated into one line. Verified against a mock-arcpy harness: group order, alias text, manifest keys, CRS repair path and `.lyrx` export all confirmed. Separately: `soil_preprocess_concept.html` given a light theme (all 115 colours moved to CSS variables, `prefers-color-scheme` default + manual toggle, every text/background pair checked to ≥4.5:1 WCAG AA); `build.py` now prunes an unpacked `CCM_Tool_v<ver>/` release folder from `os.walk()` — it would otherwise package every file in it a second time under a nested arc-path. Version bumped 0.55.1 → 0.56.0 across modules, `.pyt` + sidecars, tests, docs, manual; `CCM_Tool_v0.56.0.zip` ✅ |
