...
v0.56.0 changes
---------------
  - FIX: os.walk() descended into a release folder that had been unzipped
    *inside* the project folder (CCM_Tool_v<ver>/CCM_Tool_v<ver>/...), so
    every file in that extraction was packaged a second time under a
    nested arc-path.  It only escaped notice because the extraction
    happened to post-date the last build.  Directories whose name matches
    the release-folder pattern are now pruned from the walk, and the
    stale-artifact check reports them.
  - soil_preprocess_concept.html gained a light theme; no packaging change
    (".html" was already in INCLUDE_PATTERNS as of v0.55.1).

v0.55.1 changes
---------------
  - Two new files added to the project. QUICK_START.html (a styled HTML
    rendering of QUICK_START.md, for end users) now ships in the release
    zip: added ".html" to INCLUDE_PATTERNS. CCM_anaconda_environment.bat
    (local Anaconda-environment setup script) does NOT ship, matching the
    existing, unchanged treatment of RUN_LOCAL_VERIFICATION.bat — .bat
    files are local dev/test tooling, not an end-user deliverable, and
    ".bat" was and remains absent from INCLUDE_PATTERNS. See
    CHANGELOG_v0.55.md.
...
INCLUDE_PATTERNS = [
    ".py", ".pyt", ".xml", ".csv", ".lyrx", ".md", ".docx", ".html",
]
...
# v0.56.0 — a release zip unpacked inside the project folder creates
# CCM_Tool_v<ver>/CCM_Tool_v<ver>/..., and os.walk() would package every one
# of those files a second time.  Prune any such directory from the walk.
_RELEASE_DIR = re.compile(r"^CCM_Tool_v\d+\.\d+(?:\.\d+)?$")
...
def main():
    ...
    nested = [d for d in sorted(os.listdir(ROOT))
              if os.path.isdir(os.path.join(ROOT, d)) and _RELEASE_DIR.match(d)]
    if nested:
        print("  NOTE     unpacked release folder(s) found and EXCLUDED from")
        print("           the zip (delete them to keep the project tidy):")
        for d in nested:
            print(f"           {d}/")
    else:
        print("  OK       no unpacked release folder inside the project")
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for dirpath, dirnames, filenames in os.walk(ROOT):
            ...
            dirnames[:] = [
                ...
                and not _RELEASE_DIR.match(d)      # v0.56.0
            ]
