...
*Last updated: 2026-08-19 (v0.57 post-review fixes) | Current version: v0.57*

## Completed (v0.57 integration)

| Task | Result |
|---|---|
| Isolated integration copy | Created `CCM_Tool_v0.57`; original v0.55.1 and v0.56.4 folders remain unchanged. |
| Step 0b registration | Added the validated Data Intelligence tool between Step 0 and Step 1. |
| Project hand-off | Added additive `data_root` and `data_catalog_json` config defaults. |
| Operator setup | Added `CCM_anaconda.bat`, logged scan/test launchers, and the ArcPy smoke launcher. |
| Documentation | Updated README, manual, integration guide, status, tasks, changelog, and Quick Start HTML/Markdown. |
| Verification | Combined legacy regression tests with the v0.57 Data Intelligence and end-to-end suites. |

### v0.57 scope boundary

Step 0b reports factual inventory only. Data Quality, CCM Fitness, Confidence,
Readiness, automatic source selection, and substitution are deliberately not
implemented in this integration release.
...
### P2 — Validate the integrated Step 0b path in ArcGIS Pro

**File:** `tests/arcpy_smoke_test_step0b.py`

**What:** Run the v0.57 Step 0b smoke test on a licensed ArcGIS Pro machine.
The code path is ready; the current shell run is blocked only because ArcPy's
product license is not initialized.

**Blocker:** Requires ArcGIS Pro license initialization.

**How to run:**
1. Open ArcGIS Pro
2. Open Python window (Analysis → Python)
3. `exec(open(r"<path>\tests\arcpy_smoke_test_step0b.py").read())`
...
| 27 | v0.55.0 consolidation (multi-PC merge) | Two machines had diverged: `CCM_Tool_v0.54.1` (renamed/relicensed today — GPL headers standardized, old changelogs archived to `archives/CHANGELOG_HISTORY/` — but built from a PRE-v0.54.2 snapshot, so none of rows 21-26 below had landed) vs. `CCM_Tool_v0.54.7` (rows 21-26 all present and verified, but still carrying the pre-rename branding). Diffed every file between both folders. 14 of 19 modules differed only in header/version cosmetics. 5 files carried real logic differences — `build.py`, `ccm_isochrone.py`, `ccm_map_display.py`, `ccm_step2_mobility.py`, `ccm_step3_advanced.py` — plus `Symbology/*.lyrx`; in every case the v0.54.7 side was strictly ahead (confirmed `_union_license_safe()`, the Mobility-field renderer + 0-100 alpha scale, the 3-class pruned `.lyrx` legend, the ERROR 160333 in-memory-Reclassify + vector fallback, and `build.py`'s `should_include()`/stale-toolbox guard were ALL present only in the v0.54.7 line — the v0.54.1 line's `build.py` had regressed to the pre-v0.54.2 unfiltered packager, its `Vehicles_Can.csv` had regressed to the pre-v0.54.2 duplicated-at-root state, its `arcpy_smoke_test_step0b.py` lacked the `pytest.importorskip` guard so pytest collection would abort on any machine without ArcGIS Pro, and its 3 per-tool smoke tests plus `verify_v0544.py` were simply absent). The User Manual in the v0.54.1 folder was also internally inconsistent — the title/filename used the new naming convention (`CCM_Tool_v0.54.1`) but body text still instructed users to open the old pre-rename toolbox filename, which doesn't exist under that name. Resolution: v0.54.7 taken as the code base, v0.54.1's debrand/relicense treatment re-applied on top (not the reverse), version bumped 0.54.7 → 0.55.0 across modules, `.pyt` + sidecars, tests, docs, manual; `CLAUDE.md` and `CCM_Improvement_Research.md` (present only in the v0.54.1 working folder — the v0.54.7 folder was release-zip contents, which exclude internal dev docs) carried forward; one-time cleanup reports specific to today's v0.54.1 pass (`CLEANUP_STATUS_v0.54.1.md`, `CLEANUP_SUMMARY.md`, `FINAL_CLEANUP_REPORT.txt`, a stale-file deletion list, `ZIP_CONTENTS.md`) retired as superseded. New `QUICK_START.md` added. Syntax-checked (`ast.parse`) and pyflakes-scanned clean on all 19 modules + `.pyt`; pytest suite runs 165 passed / 3 skipped (arcpy-dependent) in an environment without a licensed ArcGIS Pro install, confirming the `importorskip` guard carried over correctly. `CCM_Tool_v0.55.0.zip` ✅ |
| 28 | v0.55.1 historical version bump | Added the original QUICK_START.html and local environment helper without changing toolbox behavior. Preserved as historical context; the current integrated release is documented in the v0.57 section above. |
| 29 | v0.57 post-review fixes (2026-08-19, no version bump) | Full logic/plan-conformance review against both implementation-plan documents, then fixed every finding: 4 high-severity (Step 3's silent-failure trap; the `soil_rci.csv` "Pt" case mismatch that dropped calibrated peat values; case-sensitive soil-code lookups that silently unpenalized soil; the H-4 smoke-test filename collision that made `RUN_ARCGIS_SMOKE_TEST.bat` skip the Step 2 test), 5 medium-severity (unclosed search cursor; soil validator never wired into Step 1; positional Step 3 sub-tool parameters; a dead `.pyt` `__main__` false-success path; a drifting hand-maintained release manifest in two files), and 3 plan-conformance gaps (P-1 roadmap-alignment decision recorded; P-2 Step 1 now reads back Step 0b's catalog via `_log_catalog_facts()`; P-3 deep-scan-mode decision recorded — not built, no current consumer). Shipped the review's automation/simplification items: `ccm_version.py` + `bump_version.py` (single-source version + one-command bump), `ccm_debug.py` (opt-in diagnostic hook for the ~105 defensive exception handlers), `ccm_data_audit.py` (calibration-data sanity checker, wired into `package_ccm_v057.py --verify-only`, 12 new tests), a guarded `tkinter` import in the GUI module (fixes pytest collection on a no-Tk Python), an `arcpy.env.workspace` save/restore fix in the SLC soil pre-processor, removal of the dead/buggy duplicate `find_latest_speed_surface()`, and renamed 5 misleadingly-version-numbered test names to `test_version_is_current`. Full detail in `CHANGELOG_v0.57.md`'s "v0.57 post-review fixes" section and `PROJECT_STATUS.md`'s top blockquote. 241 passed / 4 skipped (up from the previous 165/4 baseline — 12 new audit tests, plus the 11 dead-code tests removed and re-covered elsewhere). **Not done**: headless CLI for Steps 1-3 (5.4, explicitly deferred), further `.bat` consolidation beyond the smoke-test launcher, an actual version bump, and licensed ArcPy verification of the arcpy-dependent changes — still needs `RUN_ARCGIS_SMOKE_TEST.bat` run on a real ArcGIS Pro install per `CLAUDE.md`'s release protocol. |
