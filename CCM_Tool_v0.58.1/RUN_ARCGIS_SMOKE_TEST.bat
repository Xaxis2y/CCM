...
rem CCM Tool v0.57 - licensed ArcGIS/ArcPy smoke tests (all 5)
rem
rem v0.57 post-review "H-4": this launcher used to run only
...
rem CHANGELOG_v0.57.md "H-4".
rem
rem This script now runs all five ArcPy smoke tests (step0, step1, step2,
rem step3, step0b) and reports a combined PASS/FAIL. It also locates
...
echo [CCM v0.57] Using propy.bat: %PROPY%
...
for %%T in (step0 step1 step2 step3) do (
    echo.
    echo [CCM v0.57] ArcPy smoke test: %%T ────────────────────────────────
...
echo.
echo [CCM v0.57] ArcPy smoke test: step0b ─────────────────────────────
