...
------------------------------------------
  - VERSION now comes from the new ccm_version.py (previously read via regex
    ...
    could drift from package_ccm_v057.py's own separate "0.57" literal).
  - PY_FILES is now DERIVED from package_ccm_v057.CODE_FILES instead of
    being a second, hand-maintained list. The two had already drifted:
    package_ccm_v057.py listed 4 arcpy smoke tests build.py's PY_FILES did
    not, and build.py listed tests/test_ccm.py + tests/test_v050.py that
    package_ccm_v057.py's version-check list did not. See CHANGELOG_v0.57.md
...
# PY_FILES is derived from package_ccm_v057.py's own manifest ("M-5") so
...
# same way package_ccm_v057.TOOLBOX_FILENAME is (from the same VERSION), so
# the two never disagree even though each computes it locally.
from package_ccm_v057 import CODE_FILES as _PKG_CODE_FILES  # noqa: E402
