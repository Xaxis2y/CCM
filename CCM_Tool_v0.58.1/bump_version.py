...
updating every filename literal in package_ccm_v057.py — a chore the
...
Usage
-----
    python bump_version.py 0.58

What it does, in order:
  1. Refuses to run if package_ccm_v057.py --verify-only does not pass
  ...
  3. Rewrites every `VERSION = "<old>"` line (module docstring constants)
     in every file package_ccm_v057.VERSION_MODULES lists, to the new
  ...
  5. Updates package_ccm_v057.py's REQUIRED_FILES sidecar-name literals
  ...
  7. Runs package_ccm_v057.py --verify-only again AFTER the bump and
  ...
  8. Prints the release checklist reminder (this script deliberately does
     NOT do these next steps automatically):
       ...
       - Re-run package_ccm_v057.py --verify-only in a clean environment.
...
def _run_verify(label):
    print(f"\n-- {label}: package_ccm_v057.py --verify-only --------------------")
    result = subprocess.run(
        [sys.executable, "-B", str(ROOT / "package_ccm_v057.py"),
...
def _rewrite_version_line(path: Path, old: str, new: str, pattern=VERSION_LINE_RE):
    ...
    if match.group(2) != old:
        ...
        # (e.g. mid-bump re-run) — surface it rather than silently skipping.
        print(f"  WARN  {path.relative_to(ROOT)}: found version {match.group(2)!r}, "
              f"expected {old!r} — rewriting to {new!r} anyway")
...
def main(argv=None):
    ...
    parser.add_argument("new_version", help='e.g. "0.58" or "0.57.1"')
    ...
    import package_ccm_v057 as pkg
    ...
    if not args.skip_post_verify:
        ...
        for mod_name in ("ccm_version", "package_ccm_v057"):
