...
# CCM Soil Data Preprocessor — v0.46
...
# ── Version ─────────────────────────────────────────────────────────────────
# Versioning aligns with CCM_Tool_v0.50.0.pyt  (v0.50.0)
# v0.46 — TDS (NGA) + GGDM (NGA) support; shared Military Topographic UI
# v0.46 — SoilGrids 2.0 (Global) support: raster stack → USCS polygon FC
...
# Common field-name aliases that indicate a feature class already carries soil
# texture information.  Used by the generic auto-discovery path so that
# arbitrary ("other") data sources are recognised without a dedicated handler.
# Matching is case-insensitive and substring-based (e.g. "SANDTOTAL_R",
# "PCT_CLAY", "USDA_TEXTURE" all match).
_TEXTURE_FIELD_HINTS = (
    "SAND", "SILT", "CLAY",          # particle-size fractions (any spelling)
    "TSAND", "TSILT", "TCLAY",       # SoilGrids-style totals
    "TEXCL", "TEXTURE", "TEXT_CL",   # texture-class fields
    "USCS", "AASHTO",                # engineering classification codes
    "DRAINAGE", "DRAINCLASS",        # drainage class (used by Smart fill)
)


def discover_texture_fields(field_names):
    """Return the subset of *field_names* that look like soil-texture or
    soil-classification fields, using the alias list above.

    Pure-Python and arcpy-free so it can be unit-tested and reused by the
    generic auto-discovery path.  *field_names* may be any iterable of strings.
    """
    if not field_names:
        return set()
    found = set()
    for name in field_names:
        ...
        if any(hint in upper for hint in _TEXTURE_FIELD_HINTS):
            found.add(str(name))
    return found
...
def detect_source_type(fc_path, aux_folder=None):
    ...
    # Generic auto-discovery: recognise texture / classification fields from
    # arbitrary ("other") data sources via a broad alias list.  This lets the
    # tool process unknown FCs that nonetheless carry usable soil attributes
    # instead of failing outright.
    if discover_texture_fields(field_names):
        return SOURCE_GENERIC
...
def preprocess_slc(gdb_path, output_fc,
    ...
    # v0.57 post-review "5.5": this used to set arcpy.env.workspace = gdb_path
    # and then unconditionally clobber it to None afterwards, discarding
    # whatever workspace the caller had set before calling into this function
    # (see the same save/restore pattern already used elsewhere in this file,
    # e.g. around lines ~835, ~2121, ~2885). Now saved/restored via try/finally.
    _old_ws = arcpy.env.workspace
    arcpy.env.workspace = gdb_path
    try:
        fcs     = arcpy.ListFeatureClasses() or []
        tables  = arcpy.ListTables()         or []
    finally:
        arcpy.env.workspace = _old_ws
...
def preprocess_soilgrids(soilgrids_folder, output_fc,
    ...
    if is_weighted:
        # ── Parallel depth-layer loading (v0.46) ──────────────────────────────
        # Load the three SoilGrids depth layers concurrently using
        # ThreadPoolExecutor.  arcpy raster I/O is serialised internally via a
        # threading.Lock; the OS-level file reads can still overlap, giving a
        # measurable speedup on HDDs/SSDs with 3 depths × 3 variables = 9 reads.
        # NOTE: multiprocessing.Pool would give a larger speedup but arcpy
        # objects are not picklable — threads are the safe cross-platform choice.
        import threading
        ...
        def _load_depth_worker(args):
            """Load sand/silt/clay arrays for one SoilGrids depth layer."""
            depth, rset = args
            _arcpy_msg(messages, f"  [Parallel] Loading depth {depth} …")
            arrs_out, ras_out = {}, {}
            for var in ("sand", "silt", "clay"):
                with _arcpy_lock:
                    arr, ras = _load_raster_as_array(
                        rset[var], extent_fc=extent_fc, messages=messages
                    )
                arrs_out[var] = arr
                ras_out[var]  = ras
            return depth, arrs_out, ras_out

        n_workers = min(len(SOILGRIDS_TOPSOIL_DEPTHS), 3)
        depth_args = list(zip(SOILGRIDS_TOPSOIL_DEPTHS, raster_sets))

        _arcpy_msg(messages,
                   f"[Step 2]  Parallel load: {len(depth_args)} depth layers "
                   f"× 3 variables using {n_workers} threads …")

        with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as executor:
            depth_results = list(executor.map(_load_depth_worker, depth_args))

        # Combine weighted sums in depth order
        weighted_sand = weighted_silt = weighted_clay = None
        ref_ras = None

        for depth, arrs, ras_objs in depth_results:
            idx = SOILGRIDS_TOPSOIL_DEPTHS.index(depth)
            w   = weights[idx]
            _arcpy_msg(messages, f"  Accumulating depth {depth} (weight = {w} cm) …")

            if ref_ras is None:
                ref_ras = ras_objs["sand"]

            for var, target in [("sand", "wsand"), ("silt", "wsilt"), ("clay", "wclay")]:
...
class CCMSoilPreprocessTool:
    ...
    Appears in the CCM Tool Toolbox v0.46 as tool #8.
