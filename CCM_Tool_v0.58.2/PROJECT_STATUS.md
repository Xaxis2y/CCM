...
*Current: **v0.58.2** — August 19, 2026 (see `CHANGELOG_v0.58.2.md`).*

> **v0.58.2: Integrated Data Intelligence + Scoring + Recommendations** (August 19, 2026)
> Phase 1–2 complete: Quality scoring engine (8 metrics per dataset), Fitness
> scoring (role-specific per dataset), Confidence aggregation (per-role +
> model-level), Readiness checklist (9-point preprocessing validation), and
> Auto-Selection engine (composite ranking: Q30% + F40% + C20% + Coverage10%).
> Step 0b now generates 6 new JSON files + 2 HTML reports + recommendations;
> Step 1 displays recommendations and allows user override with audit logging.
> **Phase 3 in progress:** UI enhancements + documentation updates. **Phase 4
> planned:** full test suite + release.
>
> New modules: `ccm_data_quality.py` (468 LOC), `ccm_data_fitness.py` (537 LOC),
> `ccm_data_confidence.py` (304 LOC), `ccm_data_readiness.py` (564 LOC),
> `ccm_data_selector.py` (450+ LOC), `ccm_step0b_integration_v058.py` (450+ LOC),
> `ccm_step1_recommendations_ui.py` (NEW: displays recommendations + logs overrides).
> Unit tests: 32 comprehensive assertions covering all phases (32/32 passing ✅).
> Documentation: `CCM_v0.58.1_ROADMAP.md` (architecture + 4-phase schedule),
> `v0.58.1_PHASE1_STATUS.md` + `v0.58.1_PHASE2_STATUS.md` (delivery docs),
> `CHANGELOG_v0.58.1.md` (new features + roadmap).
>
> Backward-compatible: v0.57 outputs preserved, catalog JSON unchanged, existing
> tests still pass. No migration needed. Users see recommendations in Step 1;
> can accept or override. Ready for Phase 3–4 (docs + release).
...
> `package_ccm_v0582.py --verify-only`, a guarded `tkinter` import so the
...
## 4. How to verify the baseline
    ...
    python package_ccm_v0582.py --verify-only    # full blocking verifier (syntax, scope,
...
out of scope (`package_ccm_v0582.py` still hard-fails verification if that
...
called v0.58.1 (Data Confidence/Readiness) and v0.59+ (Uncertainty/Scenario/
...
`RELEASE_NAME`, imported by `package_ccm_v0582.py` and `build.py`),
...
`package_ccm_v0582.py --verify-only`).
