...
rem CCM Tool v0.58.2 - integrated Anaconda verification
set "ROOT=%~dp0"
...
echo [CCM v0.58.2] Running integrated toolbox and Data Intelligence verification...
echo Log: %LOG_FILE%
echo Artifacts: %ART_DIR%

call conda run -n "%ENV_NAME%" python -B "%ROOT%package_ccm_v0582.py" --verify-only --artifact-dir "%ART_DIR%" > "%LOG_FILE%" 2>&1
set "RC=%ERRORLEVEL%"
