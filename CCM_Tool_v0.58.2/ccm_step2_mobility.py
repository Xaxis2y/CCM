...
# NOTE (v0.57 post-review "H-2"/"H-3"): every key above is uppercase,
# including "PT" for peat (previously "Pt" — the sole mixed-case key in the
# whole codebase, which made soil_rci.csv rows typed as "PT" get silently
# re-inserted from the built-ins instead of being used — see CHANGELOG_v0.57.md
# "H-2"). All lookups against this table go through soil_factor() /
# load_rci_csv() / _init_rci_table(), which normalise incoming codes to
# upper-case, so any actual "Pt"/"pt"/"pT" value written into a soilType
# field by ccm_soil_preprocess.py still resolves correctly.
...
def load_rci_csv(csv_path=None):
    ...
    with open(csv_path, "r", encoding="utf-8-sig", newline="") as fh:
        for row in csv.DictReader(fh):
            ...
            # v0.57 post-review "H-2": normalise to upper-case at parse time.
            # soil_factor() also upper-cases the polygon soilType value it
            # looks up, so a code typed here in any case now reaches the
            # same table entry a polygon will actually query — a row typed
            # "pt" or "Pt" or "PT" is one row, not three silently-losing ones.
...
def _init_rci_table():
    ...
    if os.path.isfile(path):
        try:
            ...
            # Keys are upper-case on both sides (see "H-2"), so this can no
            # longer re-insert a built-in value UNDER a different-case key
            # than the one the CSV actually supplied.
            for code, vals in _BUILTIN_USCS_RCI.items():
                table.setdefault(code, vals)
            # v0.57 post-review "H-2": flag any CSV code the tool does not
            # otherwise recognise (typo, or an intentional custom code the
            # analyst forgot USCS_TO_SENSITIVITY_KEY / downstream tables
            # don't know about) instead of silently accepting it.
            _unknown = sorted(set(table) - set(_BUILTIN_USCS_RCI))
            if _unknown and _HAVE_ARCPY:
                arcpy.AddWarning(
                    "[Step 2] soil_rci.csv contains uscs_code value(s) not in "
                    "the built-in USCS set: " + ", ".join(_unknown) +
                    " — verify they match the soilType values your soil FC "
                    "actually uses, or they will never be looked up."
                )
            return table
...
def soil_factor(uscs_code, moisture, vci_1, vci_50, rci_table=None):
    ...
    v0.57 post-review "H-3": *uscs_code* is normalised to upper-case with
    surrounding whitespace stripped before lookup. Previously the lookup was
    an exact-string dict .get() — soil_factor("cl", "wet", ...) returned 1.0
    (full speed, unpenalised) instead of the 0.12 that "CL" returns, because
    every USCS table in this module is keyed upper-case. That silently
    dropped soil penalisation to zero on any soil FC whose soilType values
    were not already canonical upper-case USCS codes (e.g. an externally
    supplied "Pre-processed Soil FC" — see ccm_step1_setup.py, which accepts
    that input with no field-value validation).
    """
    code = str(uscs_code).strip().upper() if uscs_code is not None else None
    rci_tuple = (rci_table or USCS_RCI).get(code)
    if not rci_tuple:
...
def compute_stochastic_go(soil_code, moisture, vci_1, vci_50, slope_pct,
    ...
    # v0.57 post-review "H-3": normalise the same way soil_factor() does, so
    # this Monte Carlo path and the deterministic path never disagree about
    # whether a given soil code matched the table.
    _code  = str(soil_code).strip().upper() if soil_code is not None else None
    rci_t  = table.get(_code)
    idx    = _MOISTURE_INDEX.get((moisture or "moist").lower(), 1)
    ...
    for _ in range(n_trials):
        ...
        # Soil factor with perturbed RCI. Key the one-entry override table
        # with the SAME normalised code soil_factor() will look up (v0.57
        # post-review "H-3") — otherwise a raw mixed-case soil_code here
        # (e.g. "Pt") would build a table keyed "Pt" that soil_factor()'s
        # now-normalised "PT" lookup can never match, silently returning the
        # unpenalised default on every Monte Carlo trial.
        orig = rci_t
        pert_tuple = list(orig)
        pert_tuple[idx] = rci_pert
        f_soil = soil_factor(soil_code, moisture, v1, v50,
                             rci_table={_code: tuple(pert_tuple)})
...
def _union_license_safe(inputs, out_fc, join_attrs="ALL", scratch_ws=None):
    ...
    tests/arcpy_smoke_test_step0b.py, which failed at this exact line. Union (and
...
def build_speed_surface(project_folder, vehicle_name, moisture=None,
   ...
   # When use_spatial_moisture=True, query Open-Meteo VWC at a 3×3 grid across
    ...
    speed_area_pairs = []
    # v0.57 post-review "H-3": tally polygons whose soilType value has no
    # matching RCI table entry (after the same upper-case/strip normalisation
    # soil_factor() applies). Previously such a mismatch made soil silently
    # contribute NO penalty at all (factor 1.0) on every affected polygon,
    # with nothing in the run log to say so.
    _rci_lookup = rci_table or USCS_RCI
    _unmatched_soil = collections.Counter()
    ...
    with arcpy.da.UpdateCursor(out_fc, cursor_fields) as cur:
        for row in cur:
            ...
            if soil_code and str(soil_code).strip().upper() not in _rci_lookup:
                _unmatched_soil[str(soil_code)] += 1
    ...
    # v0.57 post-review "H-3": surface any soilType values that never matched
    # the RCI table — soil contributed NO penalty (factor 1.0) on every one
    # of these polygons.
    if _unmatched_soil:
        _total_unmatched = sum(_unmatched_soil.values())
        _top = ", ".join(f"{k!r} ({v:,})" for k, v in _unmatched_soil.most_common(10))
        arcpy.AddWarning(
            f"[Step 2] {_total_unmatched:,} polygon(s) carry a soilType value "
            f"with no RCI table entry — soil strength was NOT applied to them "
            f"(factor 1.0, i.e. unpenalised): {_top}"
            + (" …" if len(_unmatched_soil) > 10 else "")
        )
