...
class CCMStep3AdvancedTool:
    ...
    def execute(self, parameters, messages):
        ...
        # ── Per-analysis outcome tracking (v0.57 post-review "H-1") ────────────
        # Every enabled analysis that is actually attempted (module loaded,
        # prerequisites present) records True/False here. Analyses skipped
        # because they were never enabled, or because a documented
        # prerequisite is missing (already logged as its own AddWarning
        # above each block), are NOT counted as failures — only an attempt
        # that raised is. At the end of execute() this turns a silent
        # "Advanced Analysis Complete" into an arcpy.AddError() + failed
        # tool run whenever at least one requested analysis actually broke,
        # instead of ArcGIS Pro reporting green while every layer is missing.
        _results = []
        ...
        # Track all output FCs so we can load them into CCM_TOOL_MAP at the end.
        ...
        if run_rm:
            if _CCMReasonMapTool is None:
                ...
                try:
                    # v0.57 post-review "M-3": invoked by parameter NAME via
                    # run_tool() instead of positional rm_params[0..2] — see
                    # ccm_project_config.run_tool()'s own docstring.
                    ...
                    _record("Reason Map", True)
                except Exception as exc:
                    arcpy.AddWarning(f"[Step 3] Reason Map failed: {exc}")
                    _record("Reason Map", False, str(exc))
        ...
        if run_iso:
            if _CCMIsochroneTool is None:
                ...
                if iso_latlon is None:
                    ...
                    try:
                        # v0.57 post-review "M-3": invoked by parameter NAME
                        # via run_tool() — the previous "scan for a param
                        # whose name contains 'interval' or 'time'" and
                        # "scan reversed for direction == Output" fuzzy
                        # matches are no longer needed; ccm_isochrone.py's
                        # own getParameterInfo() names them time_bands and
                        # output_fc.
        ...
        if run_vc:
            if _CCMVehicleCompareTool is None:
                ...
                try:
                    # v0.57 post-review "M-3": invoked by parameter NAME via run_tool().
                    out_vc = _derive_output_path(speed_fc, folder_path, "vehicle_compare")
                    _cfg_mod.run_tool(
                        _CCMVehicleCompareTool(), messages,
                        fc_a=speed_fc,
                        name_a=name_a,
                        fc_b=fc_b,
                        name_b=name_b,
                    ...
                    _record("Vehicle Comparison", True)
                except Exception as exc:
                    arcpy.AddWarning(f"[Step 3] Vehicle Comparison failed: {exc}")
                    _record("Vehicle Comparison", False, str(exc))
        ...
        if run_obs:
            if _CCMObstacleDetectTool is None:
                ...
                try:
                    ...
                    _cfg_mod.run_tool(
                        _CCMObstacleDetectTool(), messages,
                        extent_fc=extent_fc,
                        contours_fc=obs_contours,
                        hydro_fc=obs_hydro,
                        barrier_fc=None,
                    ...
                    _record("Obstacle Detection", True)
                except Exception as exc:
                    arcpy.AddWarning(f"[Step 3] Obstacle Detection failed: {exc}")
                    _record("Obstacle Detection", False, str(exc))
            ...
            if _CCMWaypointTool is None:
                ...
                if wp_start_latlon is None or wp_end_latlon is None:
                    ...
                    try:
                        ...
                        _cfg_mod.run_tool(
                            _CCMWaypointTool(), messages,
                            speed_surface_fc=speed_fc,
                            speed_field=speed_field_wp,
                            start_point=_fmt_dd(*wp_start_latlon),
                            end_point=_fmt_dd(*wp_end_latlon),
                            output_route_fc=out_wp,
                            output_points_fc=out_wp_pts,
                        )
                        ...
                        # The tool ran to completion either way — "no passable path
                        # found" is a legitimate GIS result, not a failure.
                        _record("Waypoint Routing", True)
                    except Exception as exc:
                        arcpy.AddWarning(f"[Step 3] Waypoint Routing failed: {exc}")
                        _record("Waypoint Routing", False, str(exc))

        # ── Auto-load all outputs into CCM_TOOL_MAP (v0.51 — ccm_map_display) ──
        ...
        try:
            ...
            # v0.54.4: p[12] is the Vehicle A name, which is only populated
            # when Vehicle Compare runs.  Falling back to the raw feature-class
            # basename produced Contents-pane entries like
            #   CCM - speed_surface_leopard_moist (moist)
            #       Speed Surface - speed_surface_leopard_moist
            # Strip the speed_surface_ prefix and the trailing moisture token
            # so the group and layer read as "CCM - Leopard (moist)" instead.
            _veh_label = (p[12].valueAsText or "").strip()
            if not _veh_label:
                _b = os.path.splitext(os.path.basename(str(speed_fc or "")))[0]
                if _b.lower().startswith(_SPEED_FC_PREFIX):
                    _b = _b[len(_SPEED_FC_PREFIX):]
                _moist_tok = (moisture or "").strip().lower()
                if _moist_tok and _b.lower().endswith("_" + _moist_tok):
                    _b = _b[: -(len(_moist_tok) + 1)]
                _veh_label = _b.replace("_", " ").strip().title() or "Speed Surface"
            ...
            if _display_mod is None:
                ...
                for _fc in _display_mod.sort_for_draw_order(_map_outputs):
                    ...
                    try:
                        if _kind == "surface":
                            ...
                            # v0.54.4: kind_of() returns None for an output it
                            # does not recognise.  It used to fall through to
                            # "surface", which applied speed-surface symbology
                            # to whatever it was.  Leave it with ArcGIS Pro's
                            # default rendering and say so.
                            arcpy.AddWarning(
                                "[Step 3] Unrecognised output "
                                f"'{os.path.basename(str(_fc))}' — added with "
                                "default symbology (no CCM style matched)."
                            )
                ...
                if _added:
                    arcpy.AddMessage(
                        f"[Step 3] Added to CCM_TOOL_MAP: {', '.join(_added)}"
                    )
            ...
            arcpy.AddWarning(
                f"[Step 3] Could not auto-load outputs into CCM_TOOL_MAP: {exc}"
            )

        # \u2500\u2500 Analysis summary + hard failure (v0.57 post-review "H-1") \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
        # Previously this method always ended with "Advanced Analysis Complete"
        # regardless of how many of the enabled analyses actually raised \u2014
        # ArcGIS Pro would show a green/succeeded run even if every requested
        # output failed. Report a clear per-analysis summary and raise
        # arcpy.ExecuteError (tool fails, run shows red) whenever at least one
        # requested analysis was attempted and failed.
        if _results:
            arcpy.AddMessage("[Step 3] \u2500\u2500 Analysis summary \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500")
        ...
        _failed = [name for name, ok, _ in _results if not ok]
        if _failed:
            arcpy.AddError(
                f"[Step 3] {len(_failed)} of {len(_results)} requested "
                f"analysis/analyses failed: {', '.join(_failed)}"
            )
            raise arcpy.ExecuteError(
                f"Step 3 completed with {len(_failed)} failed analysis/analyses: "
                f"{', '.join(_failed)}. See the messages above for details."
            )
