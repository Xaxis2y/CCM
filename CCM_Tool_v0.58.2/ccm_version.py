...
manifests in build.py and package_ccm_v0582.py that no longer agreed; see
...
`VERSION = "0.57"` line, and package_ccm_v0582.py's static_checks() still
verifies every one of them says the same thing. What changes is *tooling*:

  - package_ccm_v0582.py and build.py both import VERSION from here instead
    ...
    package_ccm_v0582.py's, so there is exactly one list to maintain instead
...
afterwards: package_ccm_v0582.py --verify-only in a clean environment, then
