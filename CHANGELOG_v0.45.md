...
- `run_tool(tool, messages, **kwargs)` in `ccm_project_config.py` — programmatic
  ...
  multi-value support.
- Stub-tool factory in `MCE_CCM_V2.pyt` — failed module imports now surface as a
...
- **File truncation/corruption** (cause of "files truncated at the end"):
  - `MCE_CCM_V2.pyt` was cut off mid-`main()` (invalid UTF-8 tail); reconstructed.
...
- User manual updated and renamed to `MCE_CCM_Tool_v0.45_User_Manual.docx`
