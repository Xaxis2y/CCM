...
# -*- coding: utf-8 -*-
"""
MCE_CCM_v0.46.pyt — ArcGIS Python Toolbox entry point
====================================================
Cross-Country Mobility (CCM) Assessment Tool  v0.46

Toolbox exposes three tool steps:
  Step 1  — Project Setup & Pre-process   (CCMStep1SetupTool)
  Step 3  — Advanced Analysis             (CCMStep3AdvancedTool)
  Step 5  — Vehicle Comparison            (CCMVehicleCompareTool)
...
try:
    ...
    arcpy.AddWarning(f"[MCE_CCM] ccm_step1_setup not loaded: {_e}")

try:
    from ccm_step2_mobility import CCMStep2MobilityTool
except Exception as _e:
    CCMStep2MobilityTool = _make_stub_tool("Step 2.  Generate Mobility Map", _e)
    arcpy.AddWarning(f"[MCE_CCM] ccm_step2_mobility not loaded: {_e}")

try:
    from ccm_step3_advanced import CCMStep3AdvancedTool
except Exception as _e:
    CCMStep3AdvancedTool = _make_stub_tool("Step 3.  Advanced Analysis", _e)
    arcpy.AddWarning(f"[MCE_CCM] ccm_step3_advanced not loaded: {_e}")

try:
    from ccm_vehicle_compare import CCMVehicleCompareTool
except Exception as _e:
    CCMVehicleCompareTool = _make_stub_tool("5.  Compare Two Vehicles", _e)
    arcpy.AddWarning(f"[MCE_CCM] ccm_vehicle_compare not loaded: {_e}")
