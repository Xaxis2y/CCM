...
raster-based cost-distance approach:
  ...
  2. Run arcpy.sa.CostDistance() from the start point.
...
Requirements
------------
  - ArcGIS Pro Spatial Analyst extension (for CostDistance / raster ops)
...
VERSION = "2.29"  # v2.29 — Multi-format coordinate input (MGRS/DD/DMS/DDM/UTM) + display field
...
try:
    ...
    print(f"[CCM Isochrone] ccm_coords: {_e}")
...
def _snap_point_to_feature_class(
    ...
    with arcpy.da.SearchCursor(fc, ["SHAPE@", "SHAPE@XY"]) as cur:
        for row in cur:
            geom = row[0]
            if geom is None:
                continue
...
def _generate_isochrones_sa(
    ...
    """
    Generate isochrones using arcpy Spatial Analyst (CostDistance).
    """
    import arcpy.sa as sa

    arcpy.AddMessage("[CCM Isochrone] Using Spatial Analyst (CostDistance) method.")
    ...
    # ── Step 3: Cost distance ─────────────────────────────────────────────
    arcpy.AddMessage("[CCM Isochrone] Running CostDistance …")
...
class CCMIsochroneTool:
    ...
    def execute(self, parameters, messages):
        ...
        # Convert coordinate → (lat, lon) — accepts any supported format
        if _coords_mod is None:
            raise RuntimeError(
                "[CCM Isochrone] ccm_coords.py not loaded — "
        ...
        try:
            ...
            arcpy.AddMessage(
                f"[CCM Isochrone] Start [{fmt}]: {start_raw} → "
                f"{start_latlon[0]:.6f}°N  {start_latlon[1]:.6f}°E"
            )
        except Exception as e:
            raise RuntimeError(
                f"[CCM Isochrone] Cannot convert start coordinate '{start_raw}': {e}"
            )
