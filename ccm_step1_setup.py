...
try:
    ...
    print(f"[Step 1] ccm_soil_preprocess: {e}")
...
try:
    ...
    print(f"[Step 1] ccm_veg_preprocess: {e}")

_cfg_mod = None
try:
    import ccm_project_config as _cfg_mod
except Exception as e:
    print(f"[Step 1] ccm_project_config: {e}")
...
class CCMStep1SetupTool:
    ...
    def execute(self, parameters, messages):
        ...
        if not final_soil_fc and soil_raw:
            ...
            # Build _P params matching CCMSoilPreprocessTool parameter order
            # [0]=source, [1]=soil_fc, [2]=cmp, [3]=layer, [4]=name, [5]=slc_gdb,
            # [6]=ssurgo, [7]=hwsd_mdb, [8]=sg_folder, [9]=sg_depth,
            # [10]=mgcp_smc, [11]=mgcp_gdb, [12]=sand, [13]=silt, [14]=clay,
            # [15]=texcl, [16]=soilname, [17]=wrb,
            # [18]=extent_fc, [19]=gap_fill_code, [20]=output_fc
            soil_params = [
                _P(soil_source,    soil_source),    # 0
                _P(soil_raw,       soil_raw),        # 1
                _P(soil_cmp,       soil_cmp),        # 2
                _P(soil_layer_tbl, soil_layer_tbl),  # 3
                _P(soil_name_tbl,  soil_name_tbl),   # 4
                _P(soil_slc_gdb,   soil_slc_gdb),    # 5
                _P(soil_ssurgo,    soil_ssurgo),      # 6
                _P(soil_hwsd,      soil_hwsd),        # 7
                _P(soil_sg_folder, soil_sg_folder),   # 8
                _P(soil_sg_depth,  soil_sg_depth),    # 9
                _P("SMC",          "SMC"),            # 10 mgcp_smc (default)
                _P(None,           None),             # 11 mgcp_gdb
                _P(None,           None),             # 12 sand
                _P(None,           None),             # 13 silt
                _P(None,           None),             # 14 clay
                _P(None,           None),             # 15 texcl
                _P(None,           None),             # 16 soilname
                _P(None,           None),             # 17 wrb
                _P(extent_fc,      extent_fc),        # 18
                _P(soil_gapfill,   soil_gapfill),     # 19
                _P(soil_output_fc, soil_output_fc),   # 20
            ]

            try:
                _CCMSoilPreprocessTool().execute(soil_params, messages)
        ...
        if not final_veg_fc and veg_rasters_raw:
            ...
            # Build _P params matching CCMVegPreprocessTool parameter order
            # [0]=source_type, [1]=raster_path (multiValue), [2]=extent_fc,
            # [3]=output_fc, [4]=gap_vti, [5]=gap_spacing, [6]=gap_diam,
            # [7]=bio_folder, [8]=detected_source
            veg_params = [
                _P(None,            None),                           # 0 source_type (auto)
                _P(veg_rasters_raw, veg_rasters_raw,
                   values=_parse_multi(veg_rasters_raw)),            # 1 raster(s)
                _P(extent_fc,       extent_fc),                      # 2
                _P(veg_output_fc,   veg_output_fc),                  # 3
                _P(0.2,             "0.2"),                          # 4 gap vti
                _P(0.0,             "0.0"),                          # 5 gap spacing
                _P(0.0,             "0.0"),                          # 6 gap diam
                _P(None,            None),                           # 7 bio folder
                _P(None,            None),                           # 8 detected source
            ]

            try:
                _CCMVegPreprocessTool().execute(veg_params, messages)
