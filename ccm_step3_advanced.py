...
#
# VERSION = "2.39"
# v2.39 — Rewrote start/end point FC creation (v2.38 still placed wrong):
...
try:
    ...
    print(f"[Step 3] ccm_project_config: {e}")

_coords_mod = None
try:
    import ccm_coords as _coords_mod
except Exception as e:
    print(f"[Step 3] ccm_coords: {e}")
...
class CCMStep3AdvancedTool:
    ...
    def getParameterInfo(self):
        ...
        p_iso_speed_field.value = "Speed_kph"
        ...
        p_wp_speed_field.value = "Speed_kph"
    ...
    def execute(self, parameters, messages):
        ...
        if run_rm:
            if _CCMReasonMapTool is None:
                ...
                try:
                    ...
                    out_rm = _derive_output_path(speed_fc, folder_path, "reason_map")
                    for rp in reversed(rm_params):
                        if rp.direction == "Output":
                            rp.value = out_rm
                            break
        ...
        if run_iso:
            if _CCMIsochroneTool is None:
                ...
                speed_field   = p[9].valueAsText or "Speed_kph"
        ...
        if run_wp:
            if _CCMWaypointTool is None:
                ...
                speed_field_wp = p[22].valueAsText or "Speed_kph"
        ...
        try:
            ...
            _start_input_raw   = ""
            if run_iso and "iso_latlon" in dir() and iso_latlon:
                _start_ll_for_map = iso_latlon
                _start_input_raw  = (p[6].valueAsText or "").strip()
            elif run_wp and "wp_start_latlon" in dir() and wp_start_latlon:
