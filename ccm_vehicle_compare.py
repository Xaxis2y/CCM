...
# ---------------------------------------------------------------------------
# SECTION 3 — SPATIAL JOIN APPROACH
# ---------------------------------------------------------------------------

def _spatial_join_speeds(
    base_fc:     str,
    join_fc:     str,
    speed_field: str,
    out_fc:      str,
    ...
    """
    Spatial join join_fc speed values onto base_fc.
    Renames speed_field → speed_field_<suffix> in the output.
    """
    joined = os.path.join(scratch_gdb, f"ccm_cmp_join_{suffix}")
    ...
    # Rename speed field to avoid collision
    new_speed_name = f"{speed_field}_{suffix}"
    arcpy.management.AlterField(joined, speed_field, new_speed_name, new_speed_name)
    return joined
...
def compare_vehicles(
    ...
    with arcpy.da.UpdateCursor(output_fc, cursor_fields) as cur:
        for row in cur:
            ...
            # Mobility labels
            idx = 8
            row[6] = row[idx]     if mob_a_field and len(row) > idx     else None
            row[7] = row[idx + 1] if mob_b_field and len(row) > idx + 1 else None
            cur.updateRow(row)
    ...
    arcpy.AddMessage(f"  VEHICLE COMPARISON SUMMARY")
    arcpy.AddMessage(sep)
    arcpy.AddMessage(f"  Vehicle A : {vehicle_name_a}")
    arcpy.AddMessage(f"  Vehicle B : {vehicle_name_b}")
    arcpy.AddMessage(f"  Go threshold: {go_threshold} km/h")
    arcpy.AddMessage(sep)
    for label, count in counts.items():
        pct = 100.0 * count / total if total > 0 else 0
        arcpy.AddMessage(f"  {label:<16}  {count:>6}  ({pct:5.1f}%)")
    arcpy.AddMessage(sep)
...
# ---------------------------------------------------------------------------
# SECTION 5 — MULTI-VEHICLE COMPARISON (3+ vehicles)
# ---------------------------------------------------------------------------
...
class CCMVehicleCompareTool:
    ...
    def __init__(self):
        self.label       = "5.  Compare Two Vehicles"
        self.description = (
