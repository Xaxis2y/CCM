...
VERSION = "2.29"  # v2.29 — Multi-format coordinate input (MGRS/DD/DMS/DDM/UTM) + display fields
...
try:
    ...
    print(f"[CCM Waypoints] ccm_coords: {_e}")
...
def _build_adjacency(feat_data: dict, go_threshold: float) -> dict:
    ...
    Returns {oid: [(neighbour_oid, travel_time_s), ...]}

    Travel time between two polygons = distance_between_centroids /
                                        average_speed_m_per_s
    """
    adj: Dict[int, List[Tuple[int, float]]] = {oid: [] for oid in feat_data}
    ...
    def execute(self, parameters, messages):
        ...
        if veh_csv and os.path.exists(veh_csv):
            try:
                ...
                arcpy.AddWarning(f"[CCM Waypoints] Could not read vehicle CSV: {e}")
